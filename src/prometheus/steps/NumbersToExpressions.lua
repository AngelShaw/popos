...
-- Lua 5.1 builds do not all convert large numeric values to hexadecimal in
-- exactly the same way.  Evaluate the restricted arithmetic tree generated by
-- this step and keep a mutation only when it preserves the original number on
-- the interpreter that is performing the obfuscation.
local function parseGeneratedNumber(value)
...
local function evaluateGeneratedExpression(node)
	if node.kind == AstKind.NumberExpression then
		return parseGeneratedNumber(node.value)
	end

	local lhs = evaluateGeneratedExpression(node.lhs)
	local rhs = evaluateGeneratedExpression(node.rhs)
	...
	elseif node.kind == AstKind.SubExpression then
		return lhs - rhs
	elseif node.kind == AstKind.ModExpression and rhs ~= 0 then
		return lhs % rhs
	end
...
local function preservesValue(node, expected)
	local success, actual = pcall(evaluateGeneratedExpression, node)
...
function NumbersToExpressions:CreateNumberExpression(val, depth)
	if depth > 0 and math.random() >= self.InternalThreshold or depth > 15 then
		...
		if format == "hex" then
			...
			local expression = Ast.NumberExpression(result)
			if preservesValue(expression, val) then
				return expression
			end
			return Ast.NumberExpression(val)
		end
		...
		if format == "scientific" then
			...
			local expression = Ast.NumberExpression(string.format("%.15ge%d", mantissa, exp))
			if preservesValue(expression, val) then
				return expression
			end
			return Ast.NumberExpression(val)
	...
	for _, generator in ipairs(generators) do
		...
		if node and preservesValue(node, val) then
			return node
		end
	end
	return Ast.NumberExpression(val)
