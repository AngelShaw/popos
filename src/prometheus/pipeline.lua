...
function Pipeline:apply(code, filename)
	...
	if(self.Seed > 0) then
		...
		local success, seed = pcall(function()
			local seedCommands = {
				"openssl rand -hex 12";
				[[powershell.exe -NoProfile -NonInteractive -Command "$rng=New-Object System.Security.Cryptography.RNGCryptoServiceProvider;$bytes=New-Object byte[] 12;$rng.GetBytes($bytes);[BitConverter]::ToString($bytes).Replace('-','').ToLowerInvariant()"]];
			};
			local seedStr;

			for _, command in ipairs(seedCommands) do
				local commandSuccess, candidate = pcall(function()
				...
				if commandSuccess and #candidate >= 8 and candidate:match("^[%da-fA-F]+$") then
					seedStr = candidate;
					break;
				end
			end

			assert(seedStr, "no secure random provider returned a hexadecimal seed");

			-- Fold every hexadecimal digit into a signed 31-bit range. Lua 5.1
			-- passes math.randomseed through luaL_checkint before calling srand;
			-- keeping the value bounded avoids distinct large seeds collapsing.
			local seedNum = 0;
			local seedModulus = 2147483647;
			for i = 1, #seedStr do
				local char = seedStr:sub(i, i):lower();
				local digit = char:match("%d") and (char:byte() - 48) or (char:byte() - 87);
				seedNum = (seedNum * 16 + digit) % seedModulus;
			end
		...
		if success then
			...
			logger:warn("Secure random providers are unavailable. Falling back to process time.");
			local fallbackSeed = (os.time() + math.floor(os.clock() * 1000000)) % 2147483647;
			math.randomseed(fallbackSeed)
		end
	end
