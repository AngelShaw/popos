...
return function(self, statement, funcDepth)
    ...
    -- Keep the conditional branch target in an ordinary register until the
    -- complete raw block ID is known.  The program counter itself may be
    -- encoded, so it must not temporarily hold a boolean false value.
    local destinationReg = self:allocRegister(false);
    self:addStatement(self:setRegister(scope, destinationReg, Ast.AndExpression(self:register(scope, tmpReg1), self:register(scope, tmpReg2))), {destinationReg}, {tmpReg1, tmpReg2}, false);
    self:freeRegister(tmpReg2);
    self:freeRegister(tmpReg1);
    tmpReg2 = self:compileExpression(Ast.NumberExpression(finalBlock.id), funcDepth, 1)[1];
    self:addStatement(self:setRegister(scope, destinationReg, Ast.OrExpression(self:register(scope, destinationReg), self:register(scope, tmpReg2))), {destinationReg}, {destinationReg, tmpReg2}, false);
    self:freeRegister(tmpReg2);
    self:addStatement(self:setRegister(scope, self.POS_REGISTER, self:register(scope, destinationReg)), {self.POS_REGISTER}, {destinationReg}, false);
    self:freeRegister(destinationReg);
