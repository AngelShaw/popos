...
return function(Compiler)
    function Compiler:programCounterKey(scope)
        scope:addReferenceToHigherScope(self.containerFuncScope, self.programCounterKeyVar);
        return Ast.VariableExpression(self.containerFuncScope, self.programCounterKeyVar);
    end

    function Compiler:encodeProgramCounterExpression(scope, value)
        local encoding = self.programCounterEncoding;
        ...
        return Ast.ModExpression(
            Ast.AddExpression(
                Ast.AddExpression(
                    Ast.MulExpression(value, Ast.NumberExpression(encoding.multiplier)),
                    Ast.NumberExpression(encoding.offset)
                ),
                self:programCounterKey(scope)
            ),
            Ast.NumberExpression(encoding.modulus)
        );
    end

    function Compiler:decodeProgramCounterExpression(scope, value)
        local encoding = self.programCounterEncoding;
        if not encoding then
            return value;
        end
        ...
        return Ast.ModExpression(
            Ast.MulExpression(
                Ast.SubExpression(
                    Ast.SubExpression(value, Ast.NumberExpression(encoding.offset)),
                    self:programCounterKey(scope)
                ),
                Ast.NumberExpression(encoding.inverse)
            ),
            Ast.NumberExpression(encoding.modulus)
        );
    end
    ...
    function Compiler:allocRegister(isVar)
        ...
        if not isVar then
            -- In legacy mode the program counter can double as a temporary
            -- register.  Encoded mode reserves it exclusively for control
            -- flow, otherwise arbitrary values would be passed to the
            -- arithmetic encoder.
            if not self.stateEncoding and not self.registers[self.POS_REGISTER] then
    ...
    function Compiler:setRegister(scope, id, val, compundArg)
        if id == self.POS_REGISTER then
            val = self:encodeProgramCounterExpression(scope, val);
        end
        if(compundArg) then
    ...
    function Compiler:pos(scope)
        ...
        return self:decodeProgramCounterExpression(scope, Ast.VariableExpression(self.containerFuncScope, self.posVar));
    end
    ...
    function Compiler:env(scope)
        if self.environmentProxy then
            scope:addReferenceToHigherScope(self.containerFuncScope, self.proxyEnvVar);
            return Ast.VariableExpression(self.containerFuncScope, self.proxyEnvVar);
        end
    ...
    function Compiler:jmp(scope, to)
        ...
        return Ast.AssignmentStatement({Ast.AssignmentVariable(self.containerFuncScope, self.posVar)}, {
            self:encodeProgramCounterExpression(scope, to)
        });
    end
    ...
    function Compiler:setPos(scope, val)
        ...
        scope:addReferenceToHigherScope(self.containerFuncScope, self.posVar);
        return Ast.AssignmentStatement({Ast.AssignmentVariable(self.containerFuncScope, self.posVar)}, {
            self:encodeProgramCounterExpression(scope, Ast.NumberExpression(val))
        });
    end
