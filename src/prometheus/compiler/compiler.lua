...
-- Block IDs are generated below 2^24.  A power-of-two modulus keeps arithmetic
-- exact in Lua's numeric representation while any odd multiplier is invertible.
local PC_ENCODING_MODULUS = 2^25;

local function modularInverse(value, modulus)
    local oldR, r = value, modulus;
    local oldT, t = 1, 0;
...
function Compiler:new(options)
    ...
    local compiler = {
        ...
        -- Every VM build receives different program-counter constants.  The
        -- emitted dispatcher observes decoded block IDs; transitions retain
        -- encoded tokens, which removes the stable raw-ID data flow.
        stateEncoding = options.StateEncoding ~= false;
        environmentProxy = options.EnvironmentProxy ~= false;
        programCounterEncoding = nil;
    ...
    if compiler.stateEncoding then
        local multiplier = math.random(1, PC_ENCODING_MODULUS - 1);
        if multiplier % 2 == 0 then
            multiplier = multiplier + 1;
        end

        compiler.programCounterEncoding = {
            modulus = PC_ENCODING_MODULUS;
            multiplier = multiplier;
            offset = math.random(0, PC_ENCODING_MODULUS - 1);
            inverse = modularInverse(multiplier, PC_ENCODING_MODULUS);
            keyMultiplier = math.random(1, PC_ENCODING_MODULUS - 1);
            keyOffset = math.random(0, PC_ENCODING_MODULUS - 1);
        };
    end
...
function Compiler:compile(ast)
    ...
    self.programCounterKeyVar = self.containerFuncScope:addVariable();
    if self.environmentProxy then
        self.proxyEnvVar = self.containerFuncScope:addVariable();
    else
        self.proxyEnvVar = nil;
    end
