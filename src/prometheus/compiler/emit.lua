...
return function(Compiler)
    ...
    function Compiler:emitContainerFuncBody()
        ...
        if self.programCounterEncoding then
            local encoding = self.programCounterEncoding;
            local keyExpression = Ast.ModExpression(
                Ast.AddExpression(
                    Ast.MulExpression(
                        Ast.LenExpression(Ast.VariableExpression(self.containerFuncScope, self.argsVar)),
                        Ast.NumberExpression(encoding.keyMultiplier)
                    ),
                    Ast.NumberExpression(encoding.keyOffset)
                ),
                Ast.NumberExpression(encoding.modulus)
            );
            table.insert(stats, Ast.LocalVariableDeclaration(self.containerFuncScope, {self.programCounterKeyVar}, {keyExpression}));

            -- Closures receive an ordinary block ID.  Encode it only after
            -- this VM instance has derived its local execution key.
                    ...
                    Ast.AddExpression(
                        Ast.AddExpression(
                            Ast.MulExpression(
                                ...
                                Ast.NumberExpression(encoding.multiplier)
                            ),
                            Ast.NumberExpression(encoding.offset)
                        ),
                        Ast.VariableExpression(self.containerFuncScope, self.programCounterKeyVar)
                    ),
                    Ast.NumberExpression(encoding.modulus)
                )
            }));
        ...
        if self.environmentProxy then
            self.containerFuncScope:addReferenceToHigherScope(self.scope, self.envVar);
            self.containerFuncScope:addReferenceToHigherScope(self.scope, self.setmetatableVar);

            local proxyMetatable = Ast.TableConstructorExpression({
                Ast.KeyedTableEntry(
                    Ast.StringExpression("__index"),
                    Ast.VariableExpression(self.scope, self.envVar)
                ),
                Ast.KeyedTableEntry(
                    Ast.StringExpression("__newindex"),
                    Ast.VariableExpression(self.scope, self.envVar)
                ),
            });
